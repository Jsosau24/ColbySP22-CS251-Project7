...
class RBF_Net:
    ...
    def avg_cluster_dist(self, data, centroids, cluster_assignments, kmeans_obj):
        ...
        for i in range(centroids.shape[0]):
            d = kmeans_obj.dist_pt_to_centroids(centroids[i], data[cluster_assignments == i])
    ...
    def initialize(self, data):
        ...
        kmeans_obj.cluster_batch( k= self.num_hidden_units, n_iter=10)
        
        self.prototypes = kmeans_obj.centroids
        self.sigmas = self.avg_cluster_dist(data, self.prototypes, kmeans_obj.data_centroid_labels, kmeans_obj)
    ...
    def hidden_act(self, data):
        ...
        '''
        arr = np.zeros((data.shape[0], self.num_hidden_units))
    ...
    def output_act(self, hidden_acts):
        ...
        '''
        h_coordinates = np.ones((hidden_acts.shape[0], 1))
        H1 = np.hstack((hidden_acts, h_coordinates))
        Z = H1 @ self.wts
